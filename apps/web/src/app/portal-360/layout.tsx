...
/**
 * Portal 360 (migration 061) — login único e compartilhado da rede
 * 360 Service Provider. Sempre em inglês: quem usa esta área é a
 * contratante nos EUA (Miami, FL), nunca a equipe da Mawi — mesma
 * decisão já tomada para o PDF final (lib/service-orders/pdf.ts).
 */
export default async function Portal360Layout({ children }: { children: ReactNode }) {
  ...
  if (!appUser) {
    return (
      <div className="flex h-screen items-center justify-center" style={{ background: '#0a0f1e' }}>
        ...
        >
          <h1 className="text-lg font-black text-white">Access not provisioned</h1>
          <p className="mt-2 text-sm text-slate-400">
            Your account <span className="font-semibold text-slate-200">{user.email}</span> was authenticated,
            but is not linked to any account yet. Please contact your Mawi representative.
          </p>
          <div className="mt-6">
            <SignOutButton />
          </div>
  ...
  // Esta área é exclusiva da conta compartilhada da 360 — o resto da
  // equipe usa /dashboard ou /portal-funcionario (guarda simétrica lá).
  if (appUser.role !== 'client' || !appUser.clientCompany) {
    redirect('/dashboard')
  }
