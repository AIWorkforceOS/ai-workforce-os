...
const STEPS = [
  {
    id: 1,
    icon: Sparkles,
    title: 'Bem-vindo ao AI Workforce OS!',
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">configuração</p>
          <h1 className="mt-0.5 text-2xl font-black tracking-tight text-slate-900">Onboarding</h1>
        </div>
        <Link href="/dashboard" className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">
          Ir ao painel →
        </Link>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl bg-white p-6"
        ...
        <div className="mb-4 flex items-center justify-between">
          <p className="text-sm font-bold text-slate-700">Progresso da configuração</p>
        ...
        <div className="mt-4 flex items-center justify-between">
          {STEPS.map(s => (
            ...
            >
              ...
              <p className="hidden text-[9px] font-bold text-slate-400 sm:block">{s.title.split(' ')[0]}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div className="overflow-hidden rounded-2xl bg-white"
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4">
              <button
                ...
                className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-500 disabled:opacity-30 hover:bg-slate-50"
              >
                ← Voltar
              </button>
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-hidden rounded-2xl bg-white"
            ...
            <div className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-black text-slate-900">Precisa de ajuda?</p>
          ...
          <div className="rounded-2xl bg-white p-5"
            style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 0 0 1px rgba(226,232,240,0.7)' }}>
            <p className="text-xs font-black uppercase tracking-widest text-slate-400">Recursos</p>
            <div className="mt-3 space-y-2">
              {[
                { label: 'Documentação completa', href: '#' },
                { label: 'Vídeo tutorial (5min)', href: '#' },
                { label: 'Suporte por e-mail', href: 'mailto:suporte@aiworkforce.com.br' },
              ].map(({ label, href }) => (
                <a key={label} href={href}
                  className="flex items-center justify-between rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-50">
                  {label}
...
function WelcomeStep(_props: StepProps) {
  return (
    <div className="space-y-5">
      <div className="rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.08) 0%, rgba(248,250,252,0.5) 100%)', border: '1px solid rgba(34,197,94,0.15)' }}>
        <p className="text-sm font-bold text-green-700">🎉 Sua conta foi criada com sucesso!</p>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {[
          ...
          <div key={step} className="rounded-xl border border-slate-200 p-4 text-center">
            <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-full text-sm font-black text-white"
...
function AgentStep({ agentName, setAgentName, agentTone, setAgentTone }: StepProps) {
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-1.5 block text-xs font-bold text-slate-500">Nome do funcionário IA</label>
          <input
            value={agentName}
            onChange={e => setAgentName(e.target.value)}
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-green-500/50 focus:ring-1 focus:ring-green-500/20"
            placeholder="Ex: Kai, Sofia, Alex..."
          />
          <p className="mt-1 text-[11px] text-slate-400">Seus clientes vão ver esse nome no WhatsApp</p>
        </div>

        <div>
          <label className="mb-1.5 block text-xs font-bold text-slate-500">Tom de comunicação</label>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <button
                ...
                style={agentTone === id
                  ? { border: '1px solid rgba(34,197,94,0.5)', background: 'rgba(34,197,94,0.08)' }
        ...
        <div>
          <label className="mb-1.5 block text-xs font-bold text-slate-500">Script de apresentação (opcional)</label>
          <textarea
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-green-500/50 focus:ring-1 focus:ring-green-500/20"
            rows={3}
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-bold text-slate-700">Seu funcionário IA</span>
        ...
        {!sent ? (
          ...
          <div className="space-y-3">
            <div className="flex justify-end">
              <div className="rounded-2xl px-4 py-2.5 text-sm" style={{ background: '#22c55e', color: '#fff' }}>
                Olá! Quero saber mais sobre os seus serviços
              </div>
            </div>
            <div className="flex items-start gap-2">
              <div className="h-7 w-7 flex-shrink-0 rounded-full flex items-center justify-center"
                style={{ background: 'linear-gradient(135deg, #22c55e, #16a34a)' }}>
                <Bot size={12} className="text-white" />
              </div>
              <div className="rounded-2xl border border-slate-200 px-4 py-2.5 text-sm text-slate-700 bg-slate-50">
            ...
            <p className="text-xs text-green-600 font-semibold">✓ Funcionou! Seu agente está respondendo corretamente.</p>
          </div>
        )}
      </div>
    </div>
...
function DoneStep(_props: StepProps) {
  return (
    <div className="space-y-5 text-center">
      ...
      <div>
        <h3 className="text-xl font-black text-slate-900">Seu funcionário IA está ativo! 🎉</h3>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <Link href="/dashboard" className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">
