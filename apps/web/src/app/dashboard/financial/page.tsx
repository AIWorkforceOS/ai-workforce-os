    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Cobranças, receitas e custos do sistema.</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          { label: 'Pago', value: totalPaidOut, icon: Wallet, color: 'text-gray-600', bg: 'bg-gray-100' },
          ...
          <div key={label} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-0.5 text-xl font-bold text-gray-900">{fmt(value)}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        {records.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="text-sm font-medium text-gray-900">Nenhum lançamento financeiro</p>
            <p className="text-sm text-gray-500">Registre cobranças, receitas e custos do sistema.</p>
            <Link href="/dashboard/financial/new" className="mt-1 rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700">
              ...
              {records.map((r) => (
                <tr key={r.id} className="border-b border-gray-50 last:border-0">
                  <td className="px-5 py-3">
                    ...
                    <p className="text-xs text-gray-400">{CATEGORY_LABEL[r.category] ?? r.category}</p>
                  </td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3">
                    <p className="text-gray-700">{r.organizations?.name ?? '—'}</p>
                    <p className="text-xs text-gray-400">{r.units?.name ?? ''}</p>
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLOR[r.status] ?? 'bg-gray-100 text-gray-500'}`}>
